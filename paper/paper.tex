...
victim recipe but collapses to~$0.50$ under paired geometry. Across five seeds,
the same MiniWAM trained on perturbation versus paired data yields paired
AUROC~$0.500$ vs~$0.994$ and closed-loop success
$0.328{\pm}0.035$ vs~$0.973{\pm}0.009$ (gap~$0.645{\pm}0.036$);
FACT/RGB/DINOv2 variants retain gaps of~$0.62$--$0.64$. Distilling
...
occlusion, and color shifts, improving over no update by $0.24$--$0.30$. A
135-trial, 6+1-axis tabletop crossing obtains $91.1\%$ success in matched cells
and $2.2\%$ off diagonal. Fix the feedback contract before using failures to
improve a policy.
...
from the training recipe do not identify geometry dependence: the required
condition $y\not\perp g\mid s$ cannot be established there. Empirical risk
minimization may ignore~$g$ and still look strong in-domain.
...
Frames are selected from the 135-trial matched action--geometry crossing.}
\label{fig:real_robot}
\end{figure}

\paragraph{Matched hardware crossing.}
On a 6+1-axis tabletop arm, we crossed three equal-radius release actions
$\{a_0,a_1,a_2\}$ with three target-box layouts $\{g_0,g_1,g_2\}$ and repeated
every cell 15 times ($3\!\times\!3\!\times\!15=135$ trials). Duration, path
length, speed profile, carry height, and release timing were matched; only the
planar action direction and visible target geometry varied. Success counts
(rows: actions; columns: layouts) were
$\left[\begin{smallmatrix}14&0&0\\1&13&0\\0&1&14\end{smallmatrix}\right]/15$,
or $41/45$ ($91.1\%$) in matched cells versus $2/90$ ($2.2\%$) off diagonal.
The $88.9$-point difference is supported by a two-sided Fisher exact test
($p<10^{-18}$); Wilson 95\% intervals are $[79.3,96.5]\%$ and $[0.6,7.7]\%$.
Each action row and geometry column contains 45 trials and nearly identical
marginal success counts (14--15), so neither identity alone explains the
diagonal interaction. Fig.~\ref{fig:real_robot} shows representative outcomes.
...
Paired geometry removes the confound at collection time, before representation
learning. AGCD gates failure-driven updates by recipe amplitude AUROC,
amplitude-controlled paired AUROC, and closed-loop selection against an
amplitude heuristic. The frozen probes and public-video audit are controls, not
claims that every WAM leaks. A failure trajectory may teach dynamics yet remain
an invalid outcome target when perturbation size predicts its label.

\paragraph{Decision consequence and calibration.}
The protocol is a data-to-decision gate, not a new backbone. A passing head can
rank, reject, or guide policy updates. Temperature scaling preserves ordering,
and conformal rejection cannot create geometry dependence; paired ranking must
therefore precede calibration. A failing head requires a new collection recipe,
not more capacity.

\paragraph{Cost and scaling.}
A complete $m\!\times\!n$ crossing diagnoses identifiability rather than tiles a
continuous action space. Sparse matched directions reduce collection by one
third with little loss except at the smallest regime (Sec.~\ref{sec:robust}).
Stratified matching scales further, but weakens randomized counterfactual
evidence to observational evidence.

\paragraph{Hardware evidence and scope.}
The real crossing tests the same contrast under actuator, grasp, and release
variability. Its nonzero diagonal failures and off-diagonal successes, together
with balanced margins, rule out scripted labels and marginal identities. The
claim remains outcome-level interaction in a controlled tabletop task, not
unrestricted deployment. AGCD retains all failures for dynamics supervision but
requires matched crossings before they supervise outcome heads or policy
updates. FactorTask is the main evidence; dynamic-contact, cross-engine, and
hardware studies are external checks. Controlled markers and the negative
BridgeData audit do not establish broad sim-to-real transfer. The artifact
contains generators, JSON gates, scripts, tests, and the hardware matrix.
...
to an evaluator-free policy. A 135-trial real-tabletop crossing tests the same
interaction physically. AGCD is a falsifiable data-to-decision-to-learning
