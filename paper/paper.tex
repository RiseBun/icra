...
\begin{table}[t]
\centering
\caption{Noise ladder used for Fig.~\ref{fig:ladder}.}
\label{tab:ladder}
\setlength{\tabcolsep}{3pt}
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}lccccc@{}}
\toprule
\rowcolor{hdr}
$\sigma$ & 0.00 & 0.02 & 0.04 & 0.06 & 0.08 \\
\midrule
Success & 1.00 & 0.76 & 0.38 & 0.22 & 0.14 \\
Mean L2 & 0.005 & 0.036 & 0.071 & 0.106 & 0.141 \\
\bottomrule
\end{tabular*}
\end{table}
...
\begin{center}
\IfFileExists{figures/real_robot_tabletop.pdf}{%
  \includegraphics[width=0.96\linewidth,height=1.35cm,keepaspectratio]{figures/real_robot_tabletop.pdf}%
}{%
  \fbox{\parbox[c][1.15cm][c]{0.94\linewidth}{\centering\footnotesize
  Reserved real-tabletop slot: scene, candidate-action overlay, and outcome.}}
}
\end{center}
\noindent\textit{Real-tabletop transfer (reserved slot).} The same candidate
action is evaluated under different object layouts; visible geometry determines
whether the controller executes, rejects, or requests a retry.
...
\paragraph{Why a protocol, not a loss.}
A consistency regularizer can penalize a \emph{representation} of a confound
that still lives in the labels. Paired geometry removes the confound from the
labels: if $s$ is constant across~$y$, there is nothing to ``forget.'' That is
why AGCD does not propose a new training objective. CoCo remains the right tool
for visual inertia; AGCD addresses label leakage upstream of representation
learning.

\paragraph{Implication for failure-driven self-improvement.}
Failure trajectories are valid evidence about consequences, but their terminal
labels need not identify the causal interaction that produced them. An
evaluator trained on a confounded recipe can therefore pass its in-recipe test
and still provide useless policy feedback. AGCD is a gate before Best-of-$N$ or
value-guided updating: intervene on geometry, verify ranking, then permit the
score to change the policy.

\paragraph{What ``action-aware'' claims must report.}
Offline AUROC alone is ambiguous. Every outcome/value head trained on perturbed
demos should report (i) recipe amplitude$\rightarrow$success AUROC, (ii) the
same score under amplitude-controlled paired geometry, and (iii) closed-loop
success against an amplitude heuristic on matched directions.

\paragraph{Practitioner checklist.}
Use AGCD as a gate before a large WAM run, not as a post-hoc excuse:
\begin{enumerate}
\item \textbf{Collect cheaply.} Build a FactorTask-scale (or equivalent)
confounded set and a paired set with layout-disjoint splits.
\item \textbf{Probe A (G1--G2).} Report amp$\rightarrow$success AUROC on both
recipes and transfer of an amplitude head. If G1--G2 fire, stop scaling the
outcome head on the confounded recipe.
\item \textbf{Change labels, not losses.} Switch to paired geometry or
magnitude-matched policy failures. Do not first invent a forgetting regularizer
while $s$ still predicts~$y$ in the labels.
\item \textbf{Probe C (G3--G4).} Retrain the \emph{same} head; require ranking
recovery and closed-loop gap $\ge 0.15$ vs.\ the amplitude heuristic on held-out
layouts.
\item \textbf{Separate perception failures.} If a trainable encoder underfits
geometry (Fig.~\ref{fig:q3_robustness}), paired training will also fail---that is a
perception bug, not evidence that the protocol is useless.
\item \textbf{Report the triple.} Publish (i)~recipe amp AUROC, (ii)~paired amp
AUROC, (iii)~closed-loop gap, or state why selective execution was not evaluated.
\end{enumerate}

\paragraph{What AGCD does not claim.}
We do not claim every WAM already implements this chain: frozen FACT/PointWorld
probes stay near chance ($\approx0.51$), nor do we claim contact-physics parity
with Isaac Sim. The claimed quantity is the \emph{recipe gap}; the victim is a
diagnostic instrument, not a leaderboard entry.

\paragraph{Threats to validity.}
FactorTask is kinematic and contact tasks remain simulated; cross-engine equality
follows a shared contract, not full physics transfer. RGB/DINO uses synthetic
markers, and no published full-chain WAM is evaluated, although
Table~\ref{tab:recipe} shows the ingredients exist separately.

\paragraph{Reproducibility.}
The \texttt{victim\_baseline/} suite ships a one-click runner, JSON gate reports,
and all model/task/engine scripts. G1--G4 require no released checkpoint; the
intentional victim is part of the artifact, not a hidden training trick.

\paragraph{Limitations and evaluation scope.}
(1)~Primary evidence is MuJoCo FactorTask; dynamic contact covers fewer engines.
(2)~Centroid/frozen DINOv2 encode geometry, while trainable CNN/ResNet underfit.
(3)~AGCD is a protocol and decision test, not a leaderboard replacement.
(4)~Exact paired hardware interventions remain future work; BridgeData is a
label audit, and reset-free matching is observational without randomized geometry.
(5)~\emph{FACT-style} denotes the scoring interface, not a cloned backbone;
fixed perception isolates the collection recipe.
...
Learning at Scale,'' \emph{CoRL}, 2023. [Online]. Available:
